...
import ImageComp from '../snippets/mediaUpload.js';
...
export default function UplaodImage() {
	...
	this.__proto__ = new Dialogue('Image');
	...
	this.__proto__.callback = function() {
		mediaSubmit(outerThis.myContext, mySrc, 'img', object.name);
	}
	
	let elementName;
	
	if (ACTIVE_ELEM.type == 'img') {
	...
	const media = el('img', { src: mySrc || 'images/webber.png' });
			
	const mediaUrl = el('input', { class: 'form-control col-span-2', placeholder: 'Paste image url here', value: mySrc, event: {
		keyup(e) {
	...
	const LayoutController = {
			...
			const elemName = el('div', { class: 'my-2' },
				el('label', { class: 'grid grid-cols-3 items-center', textContent: 'Element name:', htmlFor: 'elemName' },
					el('input', {
						...
						id: 'elemName',
						value: elementName || 'Image'
					})
				)
			);
		
			const inputComp = ImageComp('image', { class: 'form-control col-span-1' }, function(e, name) {
				if (e) {
					...
					alert('Attempting to upload a different or invalid media type!\n\nPlease upload an image file of these formats: .jpg .png .gif .webp');
				}
			});
		...
		Options() {
			...
			const div = el('div', { class: 'gallery', event: {
				click(e) {
					let img = getParentElem(e.target, 'img', 'div');
					if(img)	{
						media.attr('src', img.src);
						mediaUrl.val(img.src);
						mySrc = img.src;
					}
				}
			} }, ...Array.from({length: 10 }, (a, i) => el('li', {},
					el('img', { src: `/images/default_${i}.jpg` })
				))
			);
