...
export default function CustomizeButton() {
	this.__proto__ = new Dialogue('Button');

	const previewBtn = el('button', { textContent: 'Preview', contentEditable: true, style: { color: 'initial' } });
	...
	this.textOptions = function(arr, prop) {
		...
		_.UPDATE_CLASS_ON_SELECT({ parent: options, className: 'textPropValHoverClass' }, true, function(btn) {
			let val = btn.text();
			if (previewBtn[0].style[prop] == val) {
	...
	const { attributesElement } = AttributeMod(attributesContext, 
	{
		...
		if (attrChange) {
			for (let key in oldCtx) {
				if (!ctx[key]) previewBtn.removeAttr(key);
				delete oldCtx[key];
			}
	...
	mainComp.append(
		el('div', { class: 'flex-ajc mb-2', style: { minHeight: '8rem' } },
			el('div', { style: { backgroundColor: 'whitesmoke' } }, previewBtn)
		),
		CLICK_DROPDOWN('Attributes', attributesElement)
	);

	const TabPanelConfig = {
		tabProps: { class: 'bg-3 hov-outline-0 mt-1' },
		clickUpdCls: 'bg-2',
