...
import AudioComp from '../snippets/mediaUpload.js';
import { FaInfoCircle } from '../react-icons/fa/index.js';

export default function UplaodAudio() {
	if (!ACTIVE_ELEM.ref) return alert('Please select an element!');
	
	this.__proto__ = new Dialogue('Audio');
	...
	this.__proto__.callback = function() {
		mediaSubmit(outerThis.myContext, mySrc, 'audio', object.name);
	}
	
	let elementName;
	
	if (ACTIVE_ELEM.type == 'audio') {
	...
	const media = el('audio', { src: mySrc, controls: true });
			
	const mediaUrl = el('input', { class: 'form-control col-span-2', placeholder: 'Paste audio url here', value: mySrc, event: {
		keyup(e) {
				...
				el('label', { class: 'grid grid-cols-3 items-center', textContent: 'Element name:', htmlFor: 'elemName' },
					el('input', {
						...
						event: {
							...
							click() { console.log(media[0]) }
						},
						id: 'elemName',
						value: elementName || 'Audio'
					})
				)
			);
		
			const inputComp = AudioComp('audio', { class: 'form-control col-span-1' }, function(e, name) {
				if (e) {
